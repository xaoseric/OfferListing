...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        datacenters = orm['offers.Location'].objects.values('datacenter_name').distinct()
        for datacenter in datacenters:
            datacenter_loc = orm["offers.Datacenter"](
                name=datacenter['datacenter_name'],
                website='',
            )
            datacenter_loc.save()

        for location in orm['offers.Location'].objects.all():
            datacenter = orm["offers.Datacenter"].objects.get(name=location.datacenter_name)
            location.datacenter = datacenter
            location.save()

    def backwards(self, orm):
        raise RuntimeError("Can not migrate backwards")
    ...
    models = {
        ...
        u'offers.location': {
            ...
            'created_at': ('django.db.models.fields.DateTimeField', [], {'auto_now_add': 'True', 'blank': 'True'}),
            'datacenter': ('django.db.models.fields.related.ForeignKey', [], {'to': u"orm['offers.Datacenter']", 'null': 'True', 'blank': 'True'}),
            'datacenter_name': ('django.db.models.fields.CharField', [], {'max_length': '255'}),
            u'id': ('django.db.models.fields.AutoField', [], {'primary_key': 'True'}),
